...
<div class="min-h-screen bg-gray-50 p-4 md:p-8">
    <div class="max-w-5xl mx-auto">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Chỉnh sửa bài báo</h1>
                <p class="text-gray-500 text-sm mt-1">Cập nhật thông tin và sử dụng AI để tối ưu nội dung bài viết.</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        <div class="mb-6">
                            ...
                            <input type="text" name="title" value="{{ old('title', $article->title) }}" 
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all" 
                        ...
                        <div class="bg-blue-50/50 p-5 rounded-2xl border border-blue-100">
                            <div class="flex justify-between items-center mb-3">
                                ...
                                <button type="button" id="btn-ai-summarize" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all flex items-center gap-2 text-xs font-semibold shadow-md active:scale-95">
                                    <i class="fa fa-magic"></i> Tạo lại tóm tắt
                                </button>
                            </div>
                            <textarea name="ai_summary" id="ai_summary" rows="4" 
                                class="w-full px-4 py-3 bg-white border border-blue-200 rounded-xl outline-none text-gray-600 text-sm resize-none" 
                                placeholder="Nhấn nút phía trên để AI cập nhật tóm tắt theo nội dung mới..." readonly>{{ old('ai_summary', $article->ai_summary) }}</textarea>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <div class="mb-8">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Ảnh bìa (Thumbnail)</label>
                            <div class="relative group border-2 border-dashed border-gray-200 rounded-2xl p-4 hover:border-blue-400 transition-all text-center">
                                ...
                                <div id="thumbnail-placeholder" class="{{ $article->thumbnail ? 'hidden' : '' }}">
                                    <i class="fa fa-cloud-upload-alt text-3xl text-gray-300 group-hover:text-blue-400 mb-2"></i>
                                    <p class="text-xs text-gray-500">Kéo thả hoặc nhấn để đổi ảnh</p>
                                </div>
                                <img id="thumbnail-preview" 
                                    src="{{ $article->thumbnail ? asset('storage/' . $article->thumbnail) : '#' }}" 
                                    class="{{ $article->thumbnail ? '' : 'hidden' }} w-full h-32 object-cover rounded-xl mx-auto shadow-sm">
                            </div>
                        </div>

                        <button type="submit" class="w-full bg-blue-600 text-white font-bold py-4 rounded-xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 active:scale-[0.98]">
                            <i class="fa fa-save mr-2"></i> Cập nhật bài báo
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    // Khởi tạo CKEditor đồng bộ trang Create
    ClassicEditor.create(document.querySelector('#editor'))
        .then(newEditor => { window.editor = newEditor; })
        .catch(error => { console.error(error); });

    // Xử lý Preview Thumbnail mới
    ...
    // AI Summarize Logic (Typewriter Effect)
    document.getElementById('btn-ai-summarize').addEventListener('click', async function() {
        ...
        status.innerText = "✨ AI đang phân tích lại nội dung mới của bạn...";
        ...
        try {
            ...
            if (data.summary) {
                ...
                status.innerText = "✅ Đã cập nhật tóm tắt thành công.";
            } else { throw new Error(); }
        } catch (error) {
            status.innerText = "❌ Lỗi kết nối AI. Hãy thử lại.";
        } finally {
            btn.disabled = false;
            btn.classList.remove('opacity-50', 'cursor-not-allowed');
            btn.innerHTML = '<i class="fa fa-magic"></i> Tạo lại tóm tắt';
        }
    });
...
<style>
    .ck-editor__editable { min-height: 450px; border: none !important; padding: 1rem !important; }
